...
export default function BioAnalyzer() {
  ...
  const analyzeBioHandler = async () => {
    ...
    try {
      ...
      setAnalysis({
        ...
        recommendedBio: "Weekend warrior who once got lost for 6 hours but still made it back with an amazing sunset photo. Looking for someone who can laugh at my terrible sense of direction and enjoys spontaneous adventures. What's the weirdest thing you've seen on an adventure?"
      });
    } finally {
      setLoading(false);
    }
  ...
  const refreshBio = async () => {
    ...
    try {
      ...
      setAnalysis({
        ...
        recommendedBio: "Curious explorer of both city hidden gems and good bookstores. Believer that the best conversations happen over shared interests and genuine curiosity about others. Currently searching for someone who appreciates both planned adventures and spontaneous discoveries. What's a skill or hobby you've always wanted to try but haven't yet?"
      });
    } catch (error) {
  ...
  const copyToClipboard = () => {
    if (!analysis?.recommendedBio) return;
